...
import static Days.Day1.processInput;
...
//        day1 = new Day1();
    ...
    void checkFirstPart(String files) {
        ...
        Assertions.assertEquals(processInput(text).get(Day1Enums.END_FLOOR), Integer.parseInt(files.split(":")[1]), "Expected result is not correct");
    }
    ...
    void checkSecondPart(String files) {
        String inputFile = "test-resources/D1/" + files.split(":")[0];
        String text = inputReader.fileReader(inputFile);
        Assertions.assertEquals(processInput(text).get(Day1Enums.BASEMENT_FIRST), Integer.parseInt(files.split(":")[1]), "Expected result is not correct");
    }
}
